...
static void
reedah_source_free (GoogleSourcePtr gsource) 
{
	if (!gsource)
		return;
 ...
 */
void
reedah_source_login (GoogleSourcePtr gsource, guint32 flags) 
{ 
...
reedah_source_item_set_flag (nodePtr node, itemPtr item, gboolean newStatus)
{
	...
	reedah_source_edit_mark_starred ((GoogleSourcePtr)root->data, item->sourceId, node->subscription->source, newStatus);
...
{
	nodePtr root = node_source_root_from_node (node);
	reedah_source_edit_mark_read ((GoogleSourcePtr)root->data, item->sourceId, node->subscription->source, newStatus);
