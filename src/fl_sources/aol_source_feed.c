 ...
 */
static void
aol_source_xpath_foreach_match (const gchar* expr, xmlXPathContextPtr xpathCtxt, xpathMatchFunc func, gpointer user_data) 
{
...
void
aol_source_migrate_node(nodePtr node) 
{
...
static void
aol_source_xml_unlink_node (xmlNodePtr node, gpointer data) 
{
	xmlUnlinkNode (node);
	xmlFreeNode (node);
}
...
static void
aol_feed_subscription_process_update_result (subscriptionPtr subscription, const struct updateResult* const result, updateFlags flags)
{
	...
	if (doc) {		
		...
		while (entry) { 
			...
			aol_source_item_retrieve_status (entry, subscription, cache);
			entry = entry->next;
		}
...
static gboolean
aol_feed_subscription_prepare_update_request (subscriptionPtr subscription, 
                                                 struct updateRequest *request)
{
	debug0 (DEBUG_UPDATE, "preparing AOL reader feed subscription for update\n");
	AolSourcePtr source = (AolSourcePtr) node_source_root_from_node (subscription->node)->data; 
...
struct subscriptionType aolSourceFeedSubscriptionType = {
