 ...
 */
nodePtr
reedah_source_opml_get_node_by_source (GoogleSourcePtr gsource, const gchar *source) 
{
 ...
 */
static void
reedah_source_update_folder (xmlNodePtr match, GoogleSourcePtr gsource, nodePtr node)
{
	xmlNodePtr	xml;
	xmlChar		*label;
	const gchar	*ptitle;
...
static void
reedah_source_merge_feed (GoogleSourcePtr source, const gchar *url, const gchar *title, const gchar *id)
{
	nodePtr	node;
	GSList	*iter;
...
gboolean
reedah_source_opml_quick_update(GoogleSourcePtr gsource) 
{
...
reedah_source_opml_subscription_prepare_update_request (subscriptionPtr subscription, struct updateRequest *request)
{
	...
	if (gsource->loginState == GOOGLE_SOURCE_STATE_NONE) {
		...
		reedah_source_login ((GoogleSourcePtr) subscription->node->data, 0) ;
		return FALSE;
	}
