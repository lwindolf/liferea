...
/** search folder properties dialog private data */
...
on_propdialog_response (GtkDialog *dialog, gint response_id, gpointer user_data)
{
	...
	if (response_id == GTK_RESPONSE_OK) {
		node_set_title (ui_data->node, gtk_entry_get_text (GTK_ENTRY (liferea_dialog_lookup (GTK_WIDGET (dialog), "feedNameEntry"))));
		unused_rules = ui_data->vfolder->rules;
		ui_data->vfolder->rules = ui_data->newRules;
		ui_data->vfolder->anyMatch = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (liferea_dialog_lookup (GTK_WIDGET(dialog), "anyRuleRadioBtn")));
	} else {
	...
	iter = unused_rules;
	while (iter) {
		vfolder_remove_rule (ui_data->vfolder, (rulePtr)iter->data);
	...
	if (response_id == GTK_RESPONSE_OK) {	
		/* update vfolder */
		ui_itemlist_clear ();
		vfolder_refresh (ui_data->vfolder);
		itemlist_unload (FALSE);
		itemlist_load (ui_data->node);
		ui_node_update (ui_data->node->id);
	}
...
static void
ui_vfolder_destroy_param_widget (GtkWidget *widget, gpointer data)
{	
...
static void
ui_vfolder_setup_rule_widgets (struct changeRequest *changeRequest, rulePtr rule)
{
...
on_ruletype_changed (GtkOptionMenu *optionmenu, gpointer user_data)
{
	...
	ruleInfo = ruleFunctions + changeRequest->rule;
	rule = rule_new (changeRequest->ui_data->vfolder, ruleInfo->ruleId, "", TRUE);
 ...
 */
static void
ui_vfolder_add_rule (struct fp_vfolder_ui_data *ui_data, rulePtr rule)
{
	...
	if (!rule) {
		...
		ui_vfolder_setup_rule_widgets (selected, newRule);
		/* add the rule to the list of new rules */
		ui_data->newRules = g_slist_append (ui_data->newRules, newRule);
	}
...
void
ui_vfolder_properties (nodePtr node)
{
	...
	ui_data->vfolder = (vfolderPtr)node->data;
	ui_data->node = node;
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (liferea_dialog_lookup (vfolderdialog, ui_data->vfolder->anyMatch?"anyRuleRadioBtn":"allRuleRadioBtn")), TRUE);
