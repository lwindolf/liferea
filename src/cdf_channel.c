...
gint	getCDFFeedUnreadCount(gchar *feedkey) { 
	...
	c = (CDFChannelPtr)g_hash_table_lookup(entries, (gpointer)feedkey);
...
/*GHashTable * getFeedNsHandler(gpointer cp) { return cdf_nslist; }*/

gchar * getCDFFeedTag(gpointer cp, int tag) { return ((CDFChannelPtr)cp)->tags[tag]; }
